...
export function Dashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      {meta.ballonDor && (
        <button
          className="card px-4 py-2.5 w-full flex items-center gap-3 text-left border border-gold/30 bg-gold/5 hover:bg-gold/10"
          onClick={() => navigate(`/player/${meta.ballonDor!.playerId}`)}
        >
          <span className="text-xl" aria-hidden>🏆</span>
          <span className="text-sm">
            <span className="text-slate-400">Reigning Ballon d’Or ({meta.ballonDor.year}): </span>
            <span className="font-semibold text-white">{meta.ballonDor.name}</span>
          </span>
          <span className="text-xs text-slate-500 ml-auto">View profile ▸</span>
        </button>
      )}
