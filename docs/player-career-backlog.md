...
> **Wave 15 (club legend, live).** The legend thresholds (≈120 apps, or fewer with goals/trophies at the club) were only computed at the retirement/legacy screen; now the moment is recognised **while it happens** — the first time long service at the current club crosses the line, a "A {club} legend" milestone and feed beat fire, once per club, tracked on `legendMilestones`. Covered by a new test (fires once, never twice).

> **Wave 16 (a new manager).** The gaffer was effectively permanent (his style fixed per club). Now a manager can be replaced over the summer (a seeded ~22%/season chance): the new boss's man-management style is re-rolled, trust resets to a neutral fresh start, and the old manager's outstanding promises leave with him — with a beat that tells you what kind of manager just walked in (a youth-truster is your opening; a ruthless one couldn't care less about your past standing). Era 0 is untouched, so existing saves/behaviour are preserved. Covered by new tests.

> **Wave 17 (build your player — skill points at creation).** Creating a player is no longer picking a preset: you start from an attribute floor and **spend a pool of skill points** wherever you want, with OVR emerging live from the real weighting model. A steepening point-cost (1→4 as an attribute climbs) stops one-attribute min-maxing; ★-marked attributes show what your position is actually rated on (invest there for OVR, elsewhere for versatility). Budgets are *derived, not guessed* — a self-calibrating recommended build binary-searches to hit each archetype's target OVR for any position (keepers auto-need ~half the points), so archetypes always fit their billing; the budget also **scales with club reputation**, and each archetype **caps a single attribute** differently (Prodigy 85 → a genuine standout; Journeyman 78). A separate **mentality pool** shapes hidden temperament (Consistency / Big-Game / Professionalism / Durability). Grouped-slider UI with a live OVR + pool readout, Recommended/Reset. Deterministic; core calibration + end-to-end creation covered by tests (recommended build hits target ±1 for every position).

> **Wave 18 (match feel — flow, duel & signature moves).** The biggest anti-repetition lever: the interactive match now has a *felt arc*. A **flow meter** (0–100, "in the zone" ↔ "rattled") folds the outcomes so far — end-product heats you up, wastefulness cools you down — giving a modest probability edge, live commentary when you cross a threshold, and a visible heat bar. A **personal duel** pits the avatar against a *named* opponent all match (an attacker vs the best opposing defender, a defender vs the best striker…): a tough marker makes those direct battles harder, the ticker colours each duel win/loss, and the post-match screen + timeline carry the verdict ("You came out on top against X, 4–1"). And **signature moves** — a cheeky chip, a nutmeg, a knuckleball, a 25-yard screamer — unlock only when you're in the zone (high flow), high risk/reward with extra spectacle and a goal-of-the-season flourish. All deterministic (flow/duel fold only resolved moments), and the determinism + no-inflation tests still pass; new tests cover flow dynamics, duel tracking and marker selection.

> **Wave 19 (training with agency).** Training was a meter you watched fill; now it's hands-on. You earn **Development Points** from how you actually play (rating-based, scaled by intensity) and **spend them to grow the attributes you choose** — a ▲ on every attribute, a steepening DP cost, capped at a ceiling tied to your potential; OVR recomputes live. A weekly **training intensity** (Light / Balanced / Intense) is a real push-your-luck: Intense speeds growth and DP but drains fitness and risks a training knock; Light keeps you fresh but slows it. Reuses the creation cost-model spirit; deterministic; new tests cover DP earning, cost curve, investment (raise / deduct / ceiling / OVR recompute) and the intensity trade-offs.

> **Wave 20 (off-pitch — the good life).** Money was a number that only went up; now it *means* something. Wages/sponsors accrue into a **spendable bank balance**, and a **lifestyle catalogue** lets the avatar spend it — a watch, a supercar, a penthouse, a mansion, a superyacht, an esports team — each with a real effect on his **following, public image and morale** (flashy toys go viral but court controversy; a **charity foundation** or giving back to your boyhood club wins the public over and cuts controversy, feeding the persona system). Bought-once, surfaced on the Off-Pitch screen with the balance and an owned/affordable state. Pure & deterministic; new tests cover buying, effects, giving, affordability and the no-double-buy rule.

> **Wave 21 (identity & backstory).** Who he *is*, not just what he can do. At creation you set a **hometown**, **nationality** and the **boyhood club** you grew up supporting, pick a **signature celebration** (it plays on every goal) and up to two **superstitions**. These then follow the whole career: signing for your boyhood club is a **homecoming** (fans + morale surge); signing for their **great rivals** is a **betrayal** that permanently costs you standing with them (detected via the existing rivalries data). A minority of players (~22%) are **dual nationals** — once you're good enough both countries court you, and committing to one **closes the other door forever**. The **shirt number** is a status object: squad numbers to start, marquee shirts (1/7/9/10) earned at Key/Star, negotiable, and **stripped when you slip to Rotation**. Breaking your pre-match ritual (rare, deterministic) starts you **off-rhythm — a flow penalty** with its own ticker line. Covered by 14 new tests.

> **Wave 22 (deeper match mechanics).** Four systems that stop matches feeling alike. **Conditions**: seasonal weather, pitch state, attendance and altitude, derived deterministically per fixture — driving rain kills the first touch, wind swirls crosses and headers, a rutted January pitch at a modest ground skews control, altitude and heat drain the legs. A **hostile away end** suppresses your starting flow *unless* your big-game temperament carries you. **Scouting report**: a pre-match dossier drawn from the *actual* opposition — a keeper who commits early (dinks are on), a booked or slow-turning centre-half (run at him / go in behind), a full-back who pushes high, a high line, a deep block, legs that'll go late. Exploiting the right weakness measurably improves that choice; ignoring it costs. **Fatigue gating**: explosive options (runs in behind, take-ons, 25-yard strikes, signature moves) are flagged 🫁 and skew heavily toward failure once you're genuinely gassed — stamina becomes a live tactical currency. **Instinct vs information**: under real pressure the UI *stops telling you the risk tier* and shows fuzzy descriptors instead ("Feels on", "Hard to read"); big-game temperament and composure decide how much clarity you keep. All deterministic; 15 new tests.

> **Wave 23 (the world adapting to you).** Fame has a price. Once the avatar is genuinely dangerous (status + ability + goals), opponents stop treating him like anyone else: a **dedicated man-marker** is assigned and named, they **double up** on him in dangerous areas, and they **break his rhythm with cynical fouls** — each foul costing flow, with its own ticker line. The team-talk screen tells you when you've been singled out ("🎯 They've set up to stop you — man-marked · doubled up · targeted for fouls"), which is the point: being kicked all afternoon is a *felt* consequence of being feared. Separately, **bogey sides** emerge organically — three quiet games against the same opponent and their manager has him worked out (a standing difficulty penalty and a story beat), until he finally produces against them and breaks the hoodoo. Deterministic; 10 new tests.

> **Wave 24 (squad & dressing-room life).** The rungs and textures of belonging to a squad. A **leadership ladder** below the armband — the leadership group, then the **vice-captaincy** (reserved for a natural leader with the room behind him), then the captaincy — each with its own beat, so being handed the armband feels earned. When he *does* wear it, the **captain's team talk** is an interactive pre-match beat that lifts the whole squad's morale — and blasting a room that doesn't respect him yet **backfires**. A **marquee signing** in his position lands with a thud (morale hit + a news beat) and shifts the ladder under his feet. Moving abroad starts a **language barrier** — relationship growth runs at half speed until he becomes fluent, tracked with a meter on the Relationships hub. And **cliques** (the countrymen, the young lads, the senior pros) are detected in every dressing room, showing where he belongs and where he's on the outside. Deterministic; 13 new tests.

> **Wave 25 (contracts & career pressure).** The parts of a career that happen in offices, not on grass. **Clause negotiation**: a contract is more than a wage — appearance and goal bonuses, a loyalty payment, wage rises on appearances, image rights, a release clause. Each costs *leverage*, and your leverage is your status + ability + the manager's trust + how many clubs are circling; ask for too much and they simply say no. **Running it down (Bosman)**: in the final year you can refuse to sign, play out the deal and leave for nothing — the money is better, and your own supporters boo you every week until you go. **Training-ground exile**: hand in a transfer request and a ruthless manager can freeze you out entirely — the bomb squad, where sharpness and confidence rot week after week until you withdraw the request or force the move. Plus beats for **forced sales** and a **failed medical** that leaves a permanent question mark. Deterministic; 11 new tests.

> **Wave 26 (media & fame).** Being watched is its own career. A **pundit nemesis** — a named ex-pro who makes you his subject and takes a swing on national TV every week you're poor. You can bite back (viral, divisive), rise above it (classy, fans warm), or pin the quote above your peg (a confidence surge) — and a genuinely good run makes him **go quiet**, which is its own reward. **Social media as a system**: post humble, hyped, funny, a call-out, or the 3am post you'll regret — reach scales with the audience you've already built, big posts **go viral**, and risky ones are stored so an **old post can resurface years later**, stripped of context. **Long-form projects** gated behind real fame: a podcast, a documentary, an autobiography, the video-game cover — each pays a large fee and locks in your public image. And the day the terraces **invent a chant for you** is a genuine milestone. Deterministic; 13 new tests.

> **Wave 27 (training & development depth).** What a footballer does between matches, and what it does to him. **Bad habits** harden out of *real* repeated behaviour — cards, wasted shots, dives — into a named reputation (Hot-headed, Wasteful, Ball hog, Goes down easily, Doesn't track back) that drags on exactly the moments it should and makes **referees stop believing a diver**; each can be **trained out** over weeks of unglamorous work. **Video analysis** lets you study a moment type you keep getting wrong for a small permanent edge, with diminishing returns and a hard cap. **Body composition** (Lean / Balanced / Powerful) is a genuine trade-off across pace, strength and injury-proneness, applied to the real attributes and fully reversible. **Coaching badges** (C → B → A → Pro) unlock by age, must be taken in order, take real weeks of study, and open the dugout after retirement. **Off-season camps** — a personal trainer (sharpness, brutal), a specialist legend (development points), or an actual holiday (morale, rust). Plus a **versatility rating** that makes genuinely two-position players harder to drop. Deterministic; 15 new tests.

> **Wave 28 (adversity).** The things nobody puts on a highlight reel. **Burnout** builds from relentless minutes, low fitness, poor form under pressure and a life lived in public — and when it crosses into real trouble it surfaces a genuine choice: take a proper break, get help, or push through, each costing something different (and each clearing the worst of it). Until addressed it quietly drags on form. **Chronic injury**: a long lay-off can leave permanent damage — a hard cap on pace, more fragility, and the honest line that "he can still be a footballer, just a different one." **Off-field incidents** (a 4am photo, a driving ban, an FA charge, a training-ground bust-up) are essentially impossible for a model professional and near-inevitable for a reckless, controversial one — club fine, public fallout, and a choice between apologising, staying quiet, or denying everything. And the **spiral**: when poor form, falling trust and sinking confidence start feeding each other, it's named, it deepens, it drags on form, and climbing out earns its own beat. Deterministic; 11 new tests.

> **Wave 29 (stats & presentation).** A new **Career Stats** screen (`/career-stats`, in the "Me" nav) that makes the record legible. A **shareable career card** — name, age, club, OVR, identity tags earned in play (hometown, boyhood club, came home / crossed the divide, had his own song, caps, coaching badges), honours, and a **verdict** drawn from the real numbers ("One of the greats." / "A career of remarkable longevity." / "The story is still being written."). A **personal dashboard** of career totals (apps, goals, assists, appearance-weighted average rating, seasons, clubs, honours, G+A per game) and a **season-by-season bar chart** with the best campaign called out. **Leaderboards** ranking every named player in the league for goals, assists and average rating — with the avatar highlighted, his rank stated, and him always shown even when he's outside the top ten (cameo appearances excluded from rating tables). A **highlight reel** of milestones, and **head-to-head records** against every club he's faced, tracked from his real match history. Pure & read-only; 12 new tests.

> **Wave 30 (money & life outside).** A wage is a headline number; what reaches him is another thing. **Tax** is modelled per country (England 45%, France 49%, Brazil 27.5%, Saudi/UAE/Qatar 0%), so the Off-Pitch screen now shows **gross vs net vs the agent's cut** — and a "smaller" offer in a tax-free league can genuinely be the bigger one. **Investments**: government bonds, a property portfolio, a restaurant with your name above the door, funding an academy back home, or backing a friend-of-a-friend's startup — each with a real yield and a real annual risk of going under, resolved once a year. **The financial adviser** is a slow-burn storyline: most are quietly effective, but one in five is not, and after a couple of years he vanishes with over half the balance. **Family** comes calling once he's visibly wealthy — a brother's business idea, an uncle's mortgage, a cousin's medical bills — and saying yes costs real money while saying no costs something else. Deterministic; 14 new tests.
